...
func (s *SQLiteStore) FailByPaymentID(ctx context.Context, paymentID string) error {
	result, err := s.db.ExecContext(ctx, `
		UPDATE payments
		SET status = ?
		WHERE payment_id = ? AND status = ?
	`, replication.StatusFailed.String(), paymentID, replication.StatusPending.String())
	if err != nil {
	...
	rowsAffected, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("read fail result for payment %s: %w", paymentID, err)
	}
	if rowsAffected > 0 {
		return nil
	}
