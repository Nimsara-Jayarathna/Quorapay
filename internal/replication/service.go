...
type FollowerTransport interface {
	...
	CancelToFollower(context.Context, string, CancelRequest) (CancelResponse, error)
}
...
func (s *ReplicationService) ReplicateWithQuorum(ctx context.Context, entry LogEntry, followerBaseURLs []string) (QuorumReplicationResult, error) {
	...
	if err := s.ledger.AppendPending(ctx, entry); err != nil {
		exists, existsErr := s.ledger.ExistsByPaymentID(ctx, entry.PaymentID)
		if existsErr != nil {
...
// ReplicateCancelWithQuorum appends a pending record (idempotent) and then marks it canceled across the cluster.
func (s *ReplicationService) ReplicateCancelWithQuorum(ctx context.Context, entry LogEntry, followerBaseURLs []string) (QuorumReplicationResult, error) {
	if s.ledger == nil {
		return QuorumReplicationResult{}, fmt.Errorf("local ledger is not configured")
	}
	if len(followerBaseURLs) > 0 && s.transport == nil {
	...
	if err := s.ledger.CancelByPaymentID(ctx, entry.PaymentID); err != nil {
		return result, fmt.Errorf("cancel payment %s locally after quorum: %w", entry.PaymentID, err)
	}
	result.Committed = true
	...
	for i, baseURL := range followerBaseURLs {
		resp, err := s.transport.CancelToFollower(ctx, baseURL, cancelReq)
		if err != nil {
			result.FollowerResults[i].Error = mergeFollowerError(result.FollowerResults[i].Error, fmt.Sprintf("cancel: %s", err.Error()))
			continue
		}
		if resp.Success {
			result.FollowerResults[i].CommitAcknowledged = true
		...
		message := resp.Message
		if message == "" {
			message = "cancel not acknowledged"
		}
		result.FollowerResults[i].Error = mergeFollowerError(result.FollowerResults[i].Error, "cancel: "+message)
	}
